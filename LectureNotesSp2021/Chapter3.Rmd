...
$$ \hat{\beta_1}\pm z^*SE(\hat{\beta_1}) = (\hat{\beta_1} - z^*SE(\hat{\beta_1}), \hat{\beta_1} + z^*SE(\hat{\beta_1}))$$

&nbsp;

That is, there is approximately a 95% chance that the interval

$$\hat{\beta_1}\pm 1.96*SE(\hat{\beta_1}) = (\hat{\beta_1} - 1.96*SE(\hat{\beta_1}), \hat{\beta_1} + 1.96*SE(\hat{\beta_1})) $$
...
| `TV` | 0.0475 | 0.0027 | 17.67 | < 0.0001 |
  ...
  a) Find a 90\% confidence interval for $\beta_0$ and interpret your results.
&nbsp;
  b) Find a 90\% confidence interval for $\beta_1$ and interpret your results.
&nbsp;
...
$H_a : \text{at least one}  \beta_j  \text{not equal to zero}$

Step 2: F-statistic 570.3
Step 3: P-value < $2.2* 10^{-16}$

Step 4: Decision p_value <0.05 Reject $H_0$ There is a linear relationship between sales and predictor variables, TV, Radio and NP.
...
**Example 13:** Find a 95\% PI (Prediction Interval) for sales when regressing `Sales` onto `Radio`, and `TV`, given that 
...
In this data set we have ___ qualitative variables. Namely, Married,Student,Ethnicity,and Gender__________.
...
```
What is the baseline category for the `married` variable in this model?

&nbsp;
model:
<!--
\begin{equatoin}
\hat{Blanace} = 523.29 -5.347\times (Married_YES)  =  \begin{cases} 
     523.29 -5.347& \text{if  person is Married} \\

       523.29   & \text{if  person is not Married} 
   \end{cases}
\end{equatoin}
&nbsp;
...
```{r }
#model with log(x) transformation
model_log_term <- lm(mpg ~ I(log(horsepower)), data = Auto)
model_log_term
summary(model_log_term)
p1<- ggplot(model_log_term, aes(x = .fitted, y = .resid)) + geom_point(color = "blue") + theme_bw() 


model_sqrt_term <- lm(mpg ~ I(sqrt(horsepower)) , data = Auto)
model_sqrt_term 

summary(model_sqrt_term)
p2<- ggplot(model_sqrt_term , aes(x = .fitted, y = .resid)) + geom_point(color = "lightblue") + theme_bw()
...
model_sqrtY_term <- lm(sqrt(mpg) ~ horsepower + I(horsepower^2), data = Auto)
summary(model_sqrtY_term)

ggplot(model_sqrtY_term, aes(x = .fitted, y = .resid)) + geom_point(color = "purple") + theme_bw()
```
