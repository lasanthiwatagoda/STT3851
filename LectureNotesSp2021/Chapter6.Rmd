...
Finally, we refit our ridge regression model on the FULL DATA SET, using the value of $\lambda$ chosen by cross validation, and examine the coefiient estimates.
...
However, the lasso has a substantial advantage over ridge regression in that the resulting coefficient estimates are sparse. Here we see that _____ of the 19 coefficient estimates are exactly zero. So the lasso model with λ chosen by cross-validation contains only ____ variables.
