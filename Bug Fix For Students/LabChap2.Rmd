...
```{r}
jpeg(file= "./JPG/greenplot.jpg") 
dev.off()
```

<!-- I had no idea why plot() function was not working.  The whole document would not knit with the function present.  I decided to not put it in but I got the same output? Sorry Dr., should have reached out to you sooner with this issue. -N -->
To display the saved file as shown in Figure [1.1](greenplot.jpg), use the `include_graphics()` function from `knitr`.

```{r echo=FALSE, fig.cap = "Using knitr::include_graphics()"}
library(knitr)
include_graphics("greenplot.jpg")
```
...
```{r}
hist(Auto2$mpg, col = "red", xlab = "MPG", main = "Frequency vs MPG")
```

```{r}
hist(Auto2$mpg, col = "red", xlab = "MPG", main = "Frequency vs MPG", breaks = 15)
```
