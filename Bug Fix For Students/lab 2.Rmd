...
```{r, label ="ig", echo = FALSE, fig.cop = "Usingknitr::include_graphics()" }
...
* Create an object named `PackagesUsed`.

* Write the packages used to a `*.bib` file.

* Load the packages with `lapply()`.

* Add a `bibliography` entry to the YAML.
* Cite the package using `@R-packagename` (look at the `*.bib` file for the exact name)
* Add a `References` section header (`## References`) at the very end of the document. The references will appear (provided they are cited) after the header.
...
James, Gareth, Daniela Witten, Trevor Hastie, and Rob Tibshirani. 2021. 
  ISLR: Data for an Introduction to Statistical Learning with Applications    in r. https://www.statlearning.com.
